...
Decimal.fromString = function(str, precision) {
    if (precision !== undefined) {
        var value = Number(str) * Math.pow(10, precision);
        value = Math.round(value);
        return new Decimal(precision, value);
    }
...
Decimal.prototype.add = function(other) {
    var minPrecision = Math.min(this.precision, other.precision);

    var a = this.newPrecision(minPrecision);
    var b = other.newPrecision(minPrecision);

    var result = new Decimal(minPrecision);
...
Decimal.prototype.mult = function(other) {
    ...
    var result = new Decimal(minPrecision);
    result.value = (a.value * b.value) / Math.pow(10, minPrecision);
